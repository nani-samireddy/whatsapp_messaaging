...
from whatsapp_messaging.crud_events import on_scheduled_messages
...
- Then It takes the function name decodes it and creates a function reference to the on_scheduled_messages function with the decoded template name.
...
def __getattr__(name):
    '''
    This function checks if the function name starts with scheduled_message_handler_ and then decodes the template name and creates a function reference to the on_scheduled_messages function.
    '''
    if name.startswith("scheduled_message_handler_"):
        template_name = name.replace("scheduled_message_handler_", "")

        def scheduled_function():
            return on_scheduled_messages(decode_from_alphanumeric(template_name))
