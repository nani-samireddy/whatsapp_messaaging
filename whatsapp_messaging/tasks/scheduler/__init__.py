...
from whatsapp_messaging.controller import process_scheduled_messages
...
- Then It takes the function name decodes it and creates a function reference to the process_scheduled_messages function with the decoded template name.
...
def __getattr__(name):
    '''
    This function checks if the function name starts with scheduled_message_handler_ and then decodes the template name and creates a function reference to the process_scheduled_messages function.
    '''
    if name.startswith("scheduled_message_handler_"):
        template_name = name.replace("scheduled_message_handler_", "")

        def scheduled_function():
            return process_scheduled_messages(decode_from_alphanumeric(template_name))
